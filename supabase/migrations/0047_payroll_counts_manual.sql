-- ============================================================
-- 0047: payroll pays for a day the owner marked present.
--
-- MANUAL (0045) is a real day on the floor — the worker was there, they
-- just had no phone to prove it. Leaving it out of the worked count
-- would mean the owner presses "Mark present", the dashboard shows the
-- person as present all month, and they are quietly short-paid on
-- payday. That is the worst class of bug this system can have, so the
-- status is added to the count in the same breath as being invented.
--
-- Regenerated verbatim from the live function with one filter changed;
-- the PF / ESI / professional-tax arithmetic is untouched.
-- ============================================================

CREATE OR REPLACE FUNCTION public.fn_generate_payroll(p_branch uuid, p_month date)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  loop
    ...
    select
      count(*) filter (where status in ('VERIFIED','APP_ONLY','DEVICE_ONLY','MANUAL')) as worked,
    ...
    -- ---------------- NORMAL / CONTRACT / unset: salary Ã· 30 ----------------
